...
public class EarthSimPanel3D  extends JPanel implements GLEventListener, TileLoadListener{
...
//	private float roverphi;
	...
	public LinkedList<GpsData> RoverPath;
...
//	public double celevation;
	...
	public class MouseEventListener implements MouseListener, MouseMotionListener, MouseWheelListener{
		EarthSimPanel3D panel;	
		double sx;
		double sy;
		boolean dragging;
		int button = 0;
		
		MouseEventListener(EarthSimPanel3D panel){
			this.panel = panel;
			dragging = false;
		}
	...
	public EarthSimPanel3D() {
		...
		RoverPath = new LinkedList<GpsData>();
...
//		clat = y2lat(camera_pos.val[1]);
//		clon = x2lon(camera_pos.val[0], GeoData.adj_latitude);
//		celevation = camera_pos.val[2];
		cdata.Latitude = y2lat(camera_pos.val[1]);
		cdata.Longitude = x2lon(camera_pos.val[0], GeoData.adj_latitude);
		cdata.Elevation = camera_pos.val[2];
		//TexTileDB.fetchAllTiles(clat, clon);
		for(CameraMoveListener l : cameraListeners){
//			l.CameraMoved(clat, clon, celevation);
			l.CameraMoved(cdata);
		}
	}
	...
	public void incCLat(){
		camera_pos.val[1] += .5*camera_pos.val[2];
		recomputeViewMatrix();
	}
	...
	public void incCLon(){
		camera_pos.val[0] += .5*camera_pos.val[2];
		recomputeViewMatrix();
	}
	...
	public void incCElevation(){
		camera_pos.val[2] += .1*camera_pos.val[2];
		recomputeViewMatrix();
	}
	
	public void decCElevation(){
		camera_pos.val[2] -= .1*camera_pos.val[2];
		recomputeViewMatrix();
	}
...
//	}
	...
	public void setRoverPos(GpsData data){
		synchronized(RoverPath){
			RoverPath.add(data);
		}
		
//		roverlat = data.Latitude;
//		roverlon = data.Longitude;
//		roverphi = (float) data.Heading;
		
		rdata.Latitude = data.Latitude;
		rdata.Longitude = data.Longitude;
		rdata.Heading = (float) data.Heading;
	}
...
//		double rz = geo.get(roverlat, roverlon);
		...
		double rx = lon2x(rdata.Longitude, GeoData.adj_latitude);
		double ry = lat2y(rdata.Latitude);
		double rz = geo.get(rdata.Latitude, rdata.Longitude);
...
//		double rz = geo.get(roverlat, roverlon);
	...
	public void SetMapView(){
		...
		phi = 0;
		theta = 0;
		//camera_pos.set(rx, ry, camera_pos.val[2], 0);
		recomputeViewMatrix();
//		viewMatrix = new HTMatrix();
//		viewMatrix.Rotate(PI/4, -1, 0, 0);
...
//	}
	...
	//private GLJPanel getglJPanel() {
		if (glJPanel == null) {
		    ...
		    //glJPanel.setSize(new Dimension(1400, 1050));
...
//		double rz = geo.get(roverlat, roverlon);
		...
		if(dist < 100){ //if we're closer than 100 meters draw the rover model
			gl.glPushMatrix();
				...
				gl.glRotated(rdata.Heading, 0,0,-1);
...
//		rpos.val[1] /= -rpos.val[2];
	...
	private void drawTerrain(GL gl){
    	...
    	int targetLOD = 25 - (int) (Math.log(Math.max(1, height-600))/Math.log(2));
    	targetLOD = Math.max(1, targetLOD);
//    	System.out.println("Target LOD = " + targetLOD);
    	TexTile t = TexTileDB.get(clat, clon, targetLOD);
    	
    	while(!t.loaded && targetLOD > 0){
...
//		gl.glRotatef(Head_angle, 0, 0, 1);
	...
	private void drawWaypoint(GL gl, Waypoint way){
		...
		if(dist < 100){ //if we're closer than 100 meters draw the waypoint model
			gl.glPushMatrix();
	
				//System.out.println("wx = " + wx + " wy = " + wy);
				gl.glTranslated(wx-camera_pos.val[0], wy-camera_pos.val[1], wz);
	...
	private void drawPath(GL gl){
		if(RoverPath.size() < 2) return;
		synchronized(RoverPath){
			Iterator<GpsData> itr = RoverPath.iterator();
			gl.glColor4ub((byte)0, (byte)0, (byte)255, (byte)255);
			gl.glLineWidth(10);
			gl.glBegin(GL.GL_LINE_STRIP);
			int i = 0;
			while(itr.hasNext()){
				...
				double ax = lon2x(a.Longitude, GeoData.adj_latitude)-camera_pos.val[0];
				double ay = lat2y(a.Latitude) - camera_pos.val[1];
				double az = geo.get(a.Latitude, a.Longitude) + .5;
				
				gl.glVertex3d(ax, ay, az);
				//System.out.println(i + " Drawing " + ax + " " + ay + " " + az);
				i++;
			}
			gl.glEnd();
		}
