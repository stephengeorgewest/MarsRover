...
int init_multicast_(struct RoverNetwork* RN) { /* create what looks like an ordinary UDP socket */
	...
	(*RN).addr.sin_addr.s_addr=inet_addr((*RN).ip_address);//htonl(INADDR_ANY); 
